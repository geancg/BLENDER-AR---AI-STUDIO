...
# ---------------------------------------------------------------------------
# Renderizador de Snapshot / Thumbnail do Modelo
# ---------------------------------------------------------------------------
...
def _generate_snapshot(context, objects, output_path):
    ...
    try:
        ...
        # Cria câmera temporária com enquadramento isométrico
...
def _upload(filepath, device_id, pro_token=None, thumbnail_path=None):
    import requests
...
def _upload_thread(filepath, device_id, pro_token, thumbnail_path=None):
    try:
        _result_queue.put(("step", "uploading", 50))
        result = _upload(filepath, device_id, pro_token, thumbnail_path)
...
def _poll_result():
    ...
    if kind == "done":
        ...
        if thumb_path and os.path.exists(thumb_path):
            try:
                pcoll.load("thumb", thumb_path, "IMAGE")
            except Exception:
                pass
...
class AR_OT_Export(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Embutir thumbnail dentro do arquivo USDZ (compatibilidade oficial Apple QuickLook / iOS)
        ...
        t = threading.Thread(target=_upload_thread, args=(tmp, device_id, token, thumb_tmp), daemon=True)
        t.start()
